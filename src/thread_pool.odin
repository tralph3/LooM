...
thread_pool_init :: proc () -> (ok: bool) {
    pool_init(&THREAD_POOL, context.allocator, THREAD_POOL_THREAD_COUNT)
    pool_start(&THREAD_POOL)
...
thread_pool_deinit :: proc () {
    pool_finish(&THREAD_POOL)
    pool_destroy(&THREAD_POOL)
}

@(private="package")
thread_pool_add_task :: proc (task: Task_Proc, data: rawptr, index: int) {
    pool_add_task(&THREAD_POOL, context.allocator, task, data, index)
}


// below lies odin's thread pool implementation, slightly tweaked for
// my needs
...
@(private="file")
pool_thread_runner :: proc(t: ^thread.Thread) {
...
pool_init :: proc(pool: ^Pool, allocator: mem.Allocator, thread_count: int) {
	...
	for _, i in pool.threads {
		t := thread.create(pool_thread_runner)
...
pool_stop_task :: proc(pool: ^Pool, user_index: int, exit_code: int = 1) -> bool {
	...
	for t, i in pool.threads {
		...
		if data.task.user_index == user_index && data.task.procedure != nil {
			...
			replacement := thread.create(pool_thread_runner)
