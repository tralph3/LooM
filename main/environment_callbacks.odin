...
process_env_callback :: proc "c" (command: lr.RetroEnvironment, data: rawptr) -> bool {
    ...
    log.debugf("Processing env callback: '%s'", command)

    using lr
    #partial switch command {
        case RetroEnvironment.GetCoreOptionsVersion: env_callback_get_core_options_version(data)
        case RetroEnvironment.GetVariableUpdate: env_callback_get_variable_update(data)
        case RetroEnvironment.GetCanDupe: env_callback_get_can_dupe(data)
        case RetroEnvironment.SetPixelFormat: env_callback_set_pixel_format(data)
 ...
 * @param[in] data <tt>const unsigned*</tt>.
*/
env_callback_set_performance_level :: proc (data: rawptr) { // TODO
}
...
env_callback_set_pixel_format :: proc (data: rawptr) { // DONE
    STATE.video.pixel_format = (^lr.RetroPixelFormat)(data)^
}
